...
void createSwapchain(vkraw::VkContext& context, GLFWwindow* window)
{
    ...
    if (!swapchainRet) swapchainRet = buildSwapchainWithMode(VK_PRESENT_MODE_FIFO_KHR, true);
    if (!swapchainRet) swapchainRet = buildSwapchainWithMode(VK_PRESENT_MODE_IMMEDIATE_KHR, false);
    if (!swapchainRet) swapchainRet = buildSwapchainWithMode(VK_PRESENT_MODE_FIFO_KHR, false);
    if (!swapchainRet) {
