...
Swapchain::Swapchain(const Context& context, const PhysicalDevice& physicalDevice,
  ...
  const auto& caps = physicalDevice.surfaceCapabilities();
  uint32_t numImages = caps.minImageCount + 1;
  if (caps.maxImageCount > 0 && numImages > caps.maxImageCount) {
    numImages = caps.maxImageCount;
  }
  if (numImages < caps.minImageCount) {
    numImages = caps.minImageCount;
  }
#else
