...
// Surface
  ...
  if (window != nullptr) {
    VK_CHECK(
        glfwCreateWindowSurface(instance_, static_cast<GLFWwindow*>(window), nullptr, &surface_));
  }
#else
  (void)window;
#endif
...
#if defined(__linux__)
  // Some Linux loader/driver combinations crash when naming VkSurfaceKHR via
  // vkSetDebugUtilsObjectNameEXT, so skip surface naming.
#else
